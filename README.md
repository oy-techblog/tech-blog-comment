...
4. **🤖 AI 댓글 분석 및 모더레이션** (GEMINI_API_KEY 설정 시)
   - Gemini AI를 사용하여 댓글 자동 분류 및 톤 분석
   - Toxicity Level 평가 (0-5 단계)
   - 부적절한 표현 자동 감지 (욕설, 인신공격, 혐오 표현 등)
   - 적절한 답변 2-3가지 자동 생성
   - **Level 4 악성 댓글 자동 숨김 처리**
   - **Level 5 악성 댓글 즉시 삭제** (증거는 알림에 보존)
5. **`oy-alldev/oliveyoung-tech-blog` 저장소에 알림**
   - 해당 포스트에 대한 알림 Issue가 **이미 있으면** → 기존 Issue에 댓글 추가
   - 알림 Issue가 **없으면** → 새 Issue 생성 (작성자 멘션 포함)
   - **일반 댓글**: AI 분류, 요약, 추천 답변 포함
   - **Level 3+ 부적절한 댓글**:
     * 모더레이션 필요 알림 발송
     * 관리자 태깅 (@oy-ladygain, @oy-0nlyoung7)
     * 대응 가이드 및 권장 조치 제공
     * Level 4는 즉시 숨김 처리
     * Level 5는 즉시 삭제 (증거는 알림에 보존)
...
- **용도**:
  1. 댓글 자동 분류 (질문/피드백/감사/토론/비판/공격적 등)
  2. 댓글 톤 및 Toxicity Level 분석 (0-5 단계)
  3. 부적절한 표현 자동 감지 (욕설, 인신공격, 혐오 표현 등)
  4. AI 기반 답변 추천 (2-3가지 답변 제안)
  5. 모더레이션 조언 및 대응 가이드 제공
- **참고**: API 키가 없어도 알림 시스템은 정상 작동하며, AI 분석 및 자동 모더레이션 기능만 비활성화됩니다.
...
AI가 댓글을 분석하여 0-5 단계로 위험도를 평가합니다:

| Level | 설명 | 자동 조치 | 알림 대상 |
|-------|------|----------|----------|
| **0** | 매우 건전한 댓글 | 없음 | 작성자 |
| **1** | 건전하며 건설적 | 없음 | 작성자 |
| **2** | 부정적이지만 예의 있음 | 없음 | 작성자 |
| **3** | 무례하거나 비꼬는 표현 포함 | ⚠️ 모더레이션 알림 | 작성자 + 관리자 |
| **4** | 욕설, 비하, 인신공격 포함 | 🚨 **댓글 자동 숨김** | 작성자 + 관리자 |
| **5** | 심각한 위협, 혐오 표현 포함 | 🗑️ **댓글 즉시 삭제** (증거 보존) | 작성자 + 관리자 |
...
#### 부적절한 댓글 (Level 4 - 숨김 처리)
```markdown
🚨 모더레이션이 필요한 댓글이 감지되었습니다

**포스트:** 포스트 제목
**댓글 작성자:** 사용자명
**위험도:** Level 4 (High)
**문제점:** profanity, personal_attack

### 📋 댓글 내용
> 댓글 원문

✅ **이 댓글은 자동으로 숨김 처리되었습니다.**
...
**모더레이션 조언:**
욕설과 인신공격이 포함되어 있습니다. 답변하지 말고 숨김 상태를 유지하는 것을 권장합니다.
...
**작성자:** @작성자
**관리자:** @oy-ladygain, @oy-0nlyoung7
```

#### 심각한 댓글 (Level 5 - 즉시 삭제)
```markdown
🚨 모더레이션이 필요한 댓글이 감지되었습니다
...
**댓글 작성자:** 사용자명
**위험도:** Level 5 (High)
**문제점:** hate_speech, personal_attack, profanity

### 📋 댓글 내용
> 댓글 원문

🗑️ **이 댓글은 자동으로 삭제되었습니다.** (증거는 이 알림에 보존됨)
...
**모더레이션 조언:**
심각한 혐오 표현과 위협이 포함되어 있습니다. 댓글이 자동으로 삭제되었으며, 반복되는 경우 사용자 차단 및 GitHub Abuse Report를 권장합니다.
...
**1️⃣ 즉시 조치 (권장)**
- ✅ 댓글이 이미 자동 삭제 처리되었습니다
- 사용자 차단 고려
- GitHub Abuse Report 제출 검토
...
- [커뮤니티 가이드라인](링크)
- [사용자 차단 방법](링크)
- [GitHub Abuse Report](https://docs.github.com/en/communities/maintaining-your-safety-on-github/reporting-abuse-or-spam)
...
Level 3 이상의 부적절한 댓글이 감지되면 다음 관리자에게 자동으로 알림이 전송됩니다:
...
자세한 가이드라인은 [COMMUNITY_GUIDELINES.md](https://github.com/oy-alldev/oliveyoung-tech-blog/blob/main/COMMUNITY_GUIDELINES.md)를 참조하세요.
